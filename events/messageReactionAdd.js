  ...
  if(message.embeds.length === 1 && message.embeds[0].title === 'Pyre | Tickets' && message.embeds[0].description === 'Create a ticket to privately message our support team regarding any related issue.'){
    if(reaction.emoji.name === "🎟️"){
      if(!message.guild.channels.cache.find(c => c.name === `ticket-${usr[0]}${usr[1]}${usr[2]}${usr[3]}`)){
        ...
        let role = message.guild.roles.cache.find(r => r.name === "Team Support");
        if(!role) {
          message.guild.roles.create({data:{name: "Pyreworks Support", permissions: 0}, reason: 'The staff needs this role to see the tickets.'});
        ...
        if(!category) category = await message.guild.channels.create("tickets", {type: "category", position: 1}).catch(e => {return functions.errorEmbed(message, message.channel, "An error has occured.")});
  ...
  if(message.embeds.length === 1 && message.embeds[0].title === '🎟️ | Ticket Completed' && message.embeds[0].description === `React with \\🗑️ to close the ticket or do not react if you have other requests.`){
    if(reaction.emoji.name === "🗑️"){
      if(user.id === db.get(`ticket.${message.channel.name}.user`)){
        ...
        .setColor(color.pyre)
        .setDescription(`The author has confirmed the ticket is closed.`)
        .setTimestamp()
        .setFooter(`Pyre | Tickets`, bot.user.displayAvatarURL())
